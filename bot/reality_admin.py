...
async def panic_rotate(domain: str, port: int = _DEFAULT_PORT) -> dict:
    """«Экстренная ротация» одним действием: новые ключи + новый набор
    shortId + новый общий SNI. Для инцидента, когда неясно, что именно
    спалилось — считаем скомпрометированным всё разом, а не гадаем какую
    часть менять. set_sni() заодно сбрасывает per-host SNI-override на всех
    нодах (см. его докстринг) — это осознанно: если админ явно настраивал
    разный SNI по нодам, после паники их придётся выставить заново через
    🌍 Серверы, зато гарантированно нет разнобоя старого/нового. Функции по
    отдельности уже атомарны и проверяют себя — тут просто гоняем по порядку."""
    await regenerate_keys()
    await regenerate_short_ids()
    await set_sni(domain, port)
