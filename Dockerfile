...
COPY bot/ ./bot/
# Только пин-файлы версий и сам апгрейд-скрипт (не весь node/) — «Добавить
# сервер» отдаёт новым нодам те же проверенные версии, что и node/bootstrap.sh
# на месте, а «Обновить xray на нодах» (bot/ssh_ops.py) гоняет ИМЕННО этот
# файл удалённо по SSH — один и тот же скрипт что руками, что из бота.
COPY node/XRAY_VERSION ./node/XRAY_VERSION
COPY node/MARZBAN_NODE_IMAGE ./node/MARZBAN_NODE_IMAGE
COPY node/upgrade_xray.sh ./node/upgrade_xray.sh
